...
import { Visibility, Schedule } from '@mui/icons-material';
import { IconBuildingStore, IconSearch, IconClock, IconFilterOff, IconX, IconMapPin } from '@tabler/icons-react';
...
export default function PendingOrderRequests() {
  ...
  const fetchOrderRequests = async () => {
    try {
      setLoading(true);
      let url = `${import.meta.env.VITE_BACKEND_URL}/api/order-requests/pending?page=${page + 1}&limit=${rowsPerPage}`;
      ...
      if (result.success) {
        ...
        toast.error(result.message || 'Failed to fetch pending order requests');
      }
    } catch (error) {
      console.error('Error fetching order requests:', error);
      toast.error('Failed to fetch pending order requests');
    } finally {
      setLoading(false);
    }
  };
      ...
      <Fade in timeout={800}>
        <Box sx={{ mb: 4 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
            <IconClock size={32} color="#2196F3" />
            <Typography variant="h4" fontWeight="bold" color="text.primary">
              Pending Order Requests
            </Typography>
            <Chip 
              label={`${filteredOrderRequests.length} Pending`}
              sx={{ 
                bgcolor: '#2196F3',
                color: 'white',
          ...
          <Typography variant="body1" color="text.secondary">
            Order requests awaiting restaurant response
          </Typography>
        </Box>
      </Fade>
                ...
                {loading ? (
                    ...
                    <TableCell colSpan={selectedRestaurant?.restaurantId === 'all' ? 13 : 12} sx={{ textAlign: 'center', py: 8 }}>
                      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          {searchTerm ? 'Try adjusting your search' : 'All order requests have been processed'}
                        </Typography>
                      </Box>
                    </TableCell>
                  </TableRow>
                  ...
                  filteredOrderRequests.map((orderRequest, index) => (
                    <Fade in timeout={1200 + index * 100} key={orderRequest._id}>
                      <TableRow sx={{ '&:hover': { backgroundColor: alpha(theme.palette.warning.main, 0.02) } }}>
