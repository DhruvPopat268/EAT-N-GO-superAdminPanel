...
import { IconBuildingStore, IconSearch, IconHourglass, IconFilterOff, IconX, IconMapPin } from '@tabler/icons-react';
...
export default function WaitingOrderRequests() {
  ...
  const fetchOrderRequests = async () => {
    try {
      setLoading(true);
      let url = `${import.meta.env.VITE_BACKEND_URL}/api/order-requests/waiting?page=${page + 1}&limit=${rowsPerPage}`;
      ...
      if (result.success) {
        ...
        toast.error(result.message || 'Failed to fetch waiting order requests');
      }
    } catch (error) {
      console.error('Error fetching order requests:', error);
      toast.error('Failed to fetch waiting order requests');
    } finally {
      setLoading(false);
    }
  };
      ...
      <Fade in timeout={800}>
        <Box sx={{ mb: 4 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
            ...
            <Chip 
              label={`${filteredOrderRequests.length} Waiting`}
              sx={{ 
                bgcolor: '#FF9800',
                color: 'white',
          ...
          <Typography variant="body1" color="text.secondary">
            Order requests with specified waiting times from restaurants
          </Typography>
        </Box>
      </Fade>
                ...
                {loading ? (
                  <TableRow>
                    <TableCell colSpan={selectedRestaurant?.restaurantId === 'all' ? 13 : 12} sx={{ textAlign: 'center', py: 8 }}>
                      <ThemeSpinner message="Loading waiting order requests..." />
                    </TableCell>
                  </TableRow>
                  ...
                  <TableRow>
                    <TableCell colSpan={selectedRestaurant?.restaurantId === 'all' ? 13 : 12} sx={{ textAlign: 'center', py: 8 }}>
                      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                        <IconHourglass size={48} color={theme.palette.text.secondary} />
                        <Typography variant="h6" color="text.secondary">
                          No waiting order requests
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          {searchTerm ? 'Try adjusting your search' : 'No order requests are currently waiting'}
                        </Typography>
                      </Box>
                    </TableCell>
                  </TableRow>
                  ...
                  filteredOrderRequests.map((orderRequest, index) => (
                    <Fade in timeout={1200 + index * 100} key={orderRequest._id}>
                      <TableRow sx={{ '&:hover': { backgroundColor: alpha(theme.palette.info.main, 0.02) } }}>
