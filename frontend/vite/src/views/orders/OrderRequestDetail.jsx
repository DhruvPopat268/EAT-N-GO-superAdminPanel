...
export default function OrderRequestDetail() {
  ...
  const fetchOrderRequestDetail = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/order-requests/by-id?orderReqId=${id}`, {
        credentials: 'include'
      });
      ...
      toast.error('Failed to fetch order request details');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}>
        <ThemeSpinner message="Loading order request details..." />
      </Box>
    );
  }
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: '#f9fafb', p: 3 }}>
      <Box sx={{ maxWidth: '80rem', mx: 'auto' }}>
        ...
        <Box sx={{ display: 'flex', gap: 3 }}>
          {/* Order Request Information - Left Side */}
              ...
              <Box sx={{ p: 3 }}>
                ...
                <Stack spacing={2.5}>
                  ...
                  <Box>
                    ...
                    <Typography variant="body1" color="text.primary">
                      {orderRequest.orderType === 'dine-in' ? 'Dine-In' : 'Takeaway'}
                    </Typography>
                  </Box>
                  ...
                  {/* Pickup Timings */}
                  {orderRequest.orderType === 'takeaway' && orderRequest.takeawayTimings && (
                    <Box>
                      <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem', mb: 0.5 }}>
                        Pickup Timings
                      </Typography>
                      ...
                      <Box sx={{ flex: 1 }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                          ...
                          <Typography variant="h6" fontWeight={600} sx={{ color: '#00a63e' }}>
                            {orderRequest.currency?.symbol || '₹'}{item.itemTotal || 0}
                          </Typography>
                        </Box>
                ...
                <Box sx={{ mt: 4, pt: 3, borderTop: '1px solid #e5e7eb' }}>
                  <Stack spacing={1.5}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <Typography variant="body1" fontWeight={500} color="text.primary">
                        {orderRequest.currency?.symbol || '₹'}{orderRequest.cartTotal || 0}
                      </Typography>
                    </Box>
                    
                    {orderRequest.appliedCoupon?.savedAmount > 0 && (
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Typography variant="body1" color="error.main">
                          Discount
                        </Typography>
                        <Typography variant="body1" fontWeight={500} color="error.main">
                          - {orderRequest.currency?.symbol || '₹'}{orderRequest.appliedCoupon.savedAmount}
                        </Typography>
                      </Box>
                    )}
                    
                    {orderRequest.appliedPendingCancellationCharges > 0 && (
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Typography variant="body1" color="success.main">
                          Cancellation Charges
                        </Typography>
                        <Typography variant="body1" fontWeight={500} color="success.main">
                          + {orderRequest.currency?.symbol || '₹'}{orderRequest.appliedPendingCancellationCharges}
                        </Typography>
                      </Box>
                    )}
