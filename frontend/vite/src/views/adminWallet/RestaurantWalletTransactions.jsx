...
import { IconWallet, IconTrendingUp, IconTrendingDown, IconCurrencyRupee, IconClock, IconBuildingStore } from '@tabler/icons-react';
import ThemeSpinner from '../../ui-component/ThemeSpinner.jsx';
import { formatDateTime } from '../../utils/dateFormatter.js';

const RestaurantWalletTransactions = () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.data.length > 0) {
          setSelectedRestaurant(data.data[0]);
        }
      }
    } catch (error) {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      setLoading(true);
      const url = `${import.meta.env.VITE_BACKEND_URL}/api/restaurant-wallet/transactions/${selectedRestaurant.restaurantId}?page=${page + 1}&limit=${rowsPerPage}`;
...
export default RestaurantWalletTransactions;
